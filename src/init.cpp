...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSumcoinamount=<n> " + _("Keep N Sumcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Sumcoin is probably already running."), strDataDir));
    ...
    nAnonymizeSumcoinAmount = GetArg("-anonymizeSumcoinamount", 0);
    if(nAnonymizeSumcoinAmount > 999999) nAnonymizeSumcoinAmount = 999999;
    if(nAnonymizeSumcoinAmount < 2) nAnonymizeSumcoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeSumcoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Sumcoin Amount %d\n", nAnonymizeSumcoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Sumcoin+1000 == (.1Sumcoin+100)*10
       10Sumcoin+10000 == (1Sumcoin+1000)*10
    */
